...
class RentReminderTests(APITestCase):
    ...
    def test_owner_sees_reminder_context_but_outsider_does_not(self):
        ...
        self.assertEqual(owner_response.data["results"][0]["kind"], "RENT_REMINDER")
        self.assertEqual(owner_response.data["results"][0]["context_label"], "Loyer 2026-08")
        self.assertEqual(owner_response.data["results"][0]["house_name"], "Maison Rappels")
        self.assertEqual(owner_response.data["results"][0]["tenant_name"], "Mariam Koffi")
        self.assertIsNone(owner_response.data["results"][0]["document_id"])
        self.assertEqual(outsider_response.status_code, status.HTTP_200_OK)
        self.assertEqual(outsider_response.data["results"], [])
